...
4. Write JSON matching `SlateResponseSchema` (`server/src/engine/schema.ts`) to `data/response.json`. `game_id` is the ESPN id from the packet table. `line` is the PICKED side's own number (+4.5 for a dog, -4.5 for a favorite; the total for over/under). `price` is the packet's price for that side.
